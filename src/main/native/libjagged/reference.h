...
GIT_INLINE(jobject) git_java_reference_init(JNIEnv *env, git_reference *ref)
{
	int ref_type;
	jclass ref_class;
	...
	if (ref_type == GIT_REF_OID) {
		...
		if ((ref_class = (*env)->FindClass(env, GIT_JAVA_CLASS_REFERENCE_DIRECT)) == NULL ||
			(ref_initmethod = (*env)->GetMethodID(env, ref_class, "<init>", "(Lorg/libgit2/jagged/Oid;)V")) == NULL ||
			(target_java = git_java_oid_init(env, git_reference_target(ref))) == NULL)
			return NULL;

		return (*env)->NewObject(env, ref_class, ref_initmethod, target_java);
	} else if (ref_type == GIT_REF_SYMBOLIC) {
		jstring target_java;

		if ((ref_class = (*env)->FindClass(env, GIT_JAVA_CLASS_REFERENCE_SYMBOLIC)) == NULL ||
			(ref_initmethod = (*env)->GetMethodID(env, ref_class, "<init>", "(Ljava/lang/String;)V")) == NULL ||
